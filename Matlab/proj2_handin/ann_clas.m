...
NHiddenUnits = [1,5,10];  % Number of hidden units
NTrain = 1;
...
% Inner
    ...
    top_besthidden(k) = besthidden;
    outer_net(k) = nc_main(X_train, y_train+1, X_test, y_test+1, besthidden); %retrain on new training data with optimized units
    % Get the predicted output for the test data
...
net = outer_net(2); %print lowest error network
displayNetworkClassification(net)
saveas(gcf, sprintf('Plots/Project2/ANN_class_network_%d',2), 'epsc') 
% Display decision boundaries
 classNames_proper = {'i';'I';'E';'A';'a:';'Y';'O';'C:';'U';'u:';'3:'};
mfig('Decision Boundaries'); clf;   
dbplot(X(CV.test(2),:), y(CV.test(2)), @(X) max_idx(nc_eval(net, X))-1);
xlabel(attributeNames(1));
ylabel(attributeNames(2));
legend(classNames_proper);
saveas(gcf, sprintf('Plots/Project2/ANN_class_%d',2), 'epsc')