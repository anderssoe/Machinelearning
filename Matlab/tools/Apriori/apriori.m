...
if minConf>0
    ...
    if ~isempty(regexp(computer,'WIN', 'once'))
        [status2, Summary2] = system(['"' pth 'apriori.exe" -tr -f"," -o -n' num2str(maxRule) ' -c' num2str(minConf) ' -s' num2str(minSup) ' -v"[Conf. %0C,Sup. %0S]" "' filename '" "apriori_temp2.txt"']);
    else
        [status2, Summary2] = system(['"' pth './apriori" -tr -f"," -o -n' num2str(maxRule) ' -c' num2str(minConf) ' -s' num2str(minSup) ' -v"[Conf. %0C,Sup. %0S]" "' filename '" "apriori_temp2.txt"']);
    end
    if status2~=0
        disp(Summary2);
        error('An error occured while calling apriori');
